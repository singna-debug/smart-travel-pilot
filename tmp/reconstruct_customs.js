...
// (1) Target the entire broken Customs section from the start of the accordion
// We'll search for the comment line as a anchor
const customsAnchor = '/* ── 입국·세관 유의사항 ── */';
const currencyAnchor = '/* ── 환전 & 계산기 ── */';

const startIndex = content.indexOf(customsAnchor);
const endIndex = content.indexOf(currencyAnchor);

if (startIndex !== -1 && endIndex !== -1) {
    const fixedCustomsBlock = `/* ── 입국·세관 유의사항 ── */
                            <GuideAccordion
                                id="customs"
    ...
    content = content.substring(0, startIndex) + fixedCustomsBlock + content.substring(endIndex);
}

fs.writeFileSync(path, content, 'utf8');
console.log('Successfully reconstructed the customs block with correct syntax.');
