...
// (1) Customs Section Clean Reconstruction
const customsHeader = '<GuideAccordion\r\n                                id="customs"'; // Try to match the exact string
const customsStartMatch = content.indexOf('id="customs"');

if (customsStartMatch !== -1) {
    const openingTagEnd = content.indexOf('>', customsStartMatch) + 1;
    // We want to find the </GuideAccordion> that is actually JUST BEFORE <GuideAccordion id="currency"
    const currencyStartMatch = content.indexOf('id="currency"');
    if (currencyStartMatch !== -1) {
        // Find the LAST </GuideAccordion> BEFORE id="currency"
        const blockBeforeCurrency = content.substring(openingTagEnd, currencyStartMatch);
        const lastClosingIdx = blockBeforeCurrency.lastIndexOf('</GuideAccordion>');
        
        if (lastClosingIdx !== -1) {
            const actualClosingTagStart = openingTagEnd + lastClosingIdx;
            
            const newCustomsInner = `
            ...
            content = content.substring(0, openingTagEnd) + newCustomsInner + content.substring(actualClosingTagStart);
        }
    }
}

fs.writeFileSync(path, content, 'utf8');
console.log('Final clean-up and build fix complete for app/confirmation/[id]/page.tsx');
