...
export default function ConfirmationPage() {
        ...
        try {
            ...
            if (json.success && json.data) {
                ...
                */
                ...
                // ---- 기본 정보 (보수적 파싱 및 &nbsp; 정화) ----
                if (raw.title) setProductName(clean(raw.title));
                if (raw.destination) setDestination(clean(raw.destination));
                if (raw.duration) setDuration(clean(raw.duration));
                ...
                // ---- 항공 상세 (강제 매핑 및 &nbsp; 정화) ----
                setAirline(clean(raw.airline));
                setDepartureAirport(clean(raw.departureAirport));
                setDepartureFlightNumber(clean(raw.departureFlightNumber || raw.flightCode));
                setReturnFlightNumber(clean(raw.returnFlightNumber));
                ...
                if (raw.meetingInfo && Array.isArray(raw.meetingInfo)) {
                    const cleanedMeeting = raw.meetingInfo.map((m: any) => ({
                        ...m,
                        location: clean(m.location),
                        description: clean(m.description)
                    }));
                    setMeetingInfo(cleanedMeeting);
    ...
    const addItineraryDay = () => {
        setItinerary(prev => [
            ...prev,
            {
                ...
                title: '',
                route: '',
                timeline: [{ type: 'default', title: '', description: '' }],
                hotel: '',
    ...
    const removeItineraryDay = (idx: number) => {
        if (!confirm('해당 일자의 모든 일정이 삭제됩니다. 진행하시겠습니까?')) return;
        setItinerary(prev => prev.filter((_, i) => i !== idx).map((d, i) => ({ ...d, day: i + 1 })));
    };

    const updateDayHotel = (idx: number, hotel: string) => {
    ...
    // 미팅 정보 전용 이미지 업로드 핸들러
    const handleMeetingImageUpload = async (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
        const file = e.target.files?.[0];
        if (!file || !supabase) return;

        setUploading(`meeting-${index}`);
        try {
            const fileExt = file.name.split('.').pop();
            ...
            if (!result.success) {
                if (result.error?.includes('bucket not found')) {
                    if (confirm('저장소 버킷이 없습니다. 지금 바로 생성하시겠습니까?')) {
                        const setupRes = await fetch('/api/admin/setup-storage');
                        const setupJson = await setupRes.json();
                        if (setupJson.success) alert('버킷이 생성되었습니다. 다시 업로드해 주세요.');
                    }
                } else {
                    alert(`업로드 실패: ${result.error}`);
                }
            ...
            updateMeetingInfo(index, 'imageUrl', result.publicUrl);
            alert('이미지가 업로드되었습니다.');
        } catch (err: any) {
            ...
            <div className="confirm-section">
                ...
                {meetingInfo.map((m, i) => (
                    <div key={i} className="confirm-grid" style={{ marginBottom: '16px', padding: '16px', background: 'var(--bg-secondary)', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                        ...
                        <div className="confirm-field" style={{ gridColumn: '1 / -1' }}>
                            <label>관련 이미지 (직접 업로드 또는 URL)</label>
                            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                <input
                                    value={m.imageUrl || ''}
                                    onChange={e => updateMeetingInfo(i, 'imageUrl', e.target.value)}
                                    placeholder="https://..."
                                    style={{ flex: 1 }}
                                />
                                <div style={{ position: 'relative', overflow: 'hidden' }}>
                                    <button style={{
                                        ...
                                        {uploading === `meeting-${i}` ? '⌛ 업로드 중...' : '파일 올리기'}
                                    </button>
                                    <input
                                        type="file"
                                        accept="image/*"
            ...
            <div className="confirm-section">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                ...
                                {(day.timeline || []).map((item: any, idx: number) => (
                                    <div key={idx} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                                        ...
                                        <button
                                            ...
                                            title={item.type === 'location' ? '관광지 (📍)' : '기타/안내 (•)'}
                                        >
                                            {item.type === 'location' ? '📍' : '•'}
                                        </button>
                                        ...
                                        {/* Delete Item */}
                                        <button
                                            onClick={() => removeTimelineItem(i, idx)}
                                            style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '1.1rem', padding: '4px', marginTop: '20px', transition: 'color 0.15s' }}
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px', padding: '16px', borderRadius: '10px', background: 'var(--bg-tertiary)', border: '1px solid var(--border-color)' }}>
                                <div className="confirm-field">
                                    ...
                                    <input
                                        ...
                                        onChange={e => updateDayMeal(i, 'breakfast', e.target.value)}
                                    />
                                </div>
                                <div className="confirm-field">
                                    <label>🍱 중식</label>
                                    <input
                                        ...
                                        onChange={e => updateDayMeal(i, 'lunch', e.target.value)}
                                    />
                                </div>
                                <div className="confirm-field">
                                    <label>🍗 석식</label>
                                    <input
                                        ...
                                        onChange={e => updateDayMeal(i, 'dinner', e.target.value)}
                                    />
                                </div>
                                <div className="confirm-field">
                                    <label>🏨 숙박</label>
                    ...
                    <button
                        ...
                        className="btn-add-traveler"
                        style={{ padding: '16px', fontSize: '0.9rem', fontWeight: 600 }}
                    >
                        ➕ 새로운 여행 날짜(Day) 추가하기
                    </button>
                </div>
