...
export function inferDestination(title: string, domText?: string, url?: string): string {
    ...
    const matchRegion = (text: string): string => {
        ...
        if (/오사카|교토|도쿄|후쿠오카|삿포로|북해도|나고야|오키나와|우지|이네|시라하마|노보리베츠|오타루|도야|대마도|니가타|오부세|나가노|묘코|토카마치|도카마치|에치고유자와|야히코|일본/i.test(text)) return '일본';
    ...
    const REGION_SPOTS_MAP: Record<string, string[]> = {
        ...
        '일본': ['오사카', '교토', '우지', '이네', '도쿄', '후쿠오카', '삿포로', '북해도', '나고야', '오키나와', '노보리베츠', '오타루', '도야', '시라하마', '와카야마', '니가타', '오부세', '나가노', '묘코', '도카마치', '토카마치', '에치고유자와', '야히코', '마츠야마', '타카마츠', '이마바리'],
